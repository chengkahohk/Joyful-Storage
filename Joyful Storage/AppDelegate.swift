...
class AppDelegate: UIResponder, UIApplicationDelegate, UISplitViewControllerDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        AWSMobileClient.sharedInstance().initialize { (userState, error) in
            print ("\(String(describing: userState))")
            print ("\(String(describing: error))")
        }
        
        let dbURL = URL(fileURLWithPath: NSTemporaryDirectory() + "Pico.db")
        
        do {
            let config = try AWSAppSyncClientConfiguration(appSyncClientInfo: AWSAppSyncClientInfo(), userPoolsAuthProvider: MyAuthProvider(), databaseURL: dbURL)
